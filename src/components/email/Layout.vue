<template>
  <v-app id="mail" class="mail">
    <v-toolbar id="topbar" fixed app flat dark color="indigo" clipped-left class="mail-toolbar">
      <v-toolbar-side-icon class="hidden-sm-and-up" @click="toggleDrawer" />
      <v-avatar class="hidden-sm-and-down">
        <img src="/src/static/m.png" alt="Vue Material Mail">
      </v-avatar>
      ...
      <v-menu offset-y origin="center center" :nudge-bottom="10" transition="scale-transition">
        ...
        <v-list class="pa-0">
          <v-list-tile v-for="(item,index) in items" :key="index" :to="!item.href ? { name: item.name } : null" :href="item.href" ripple="ripple" :disabled="item.disabled" :target="item.target" rel="noopener" @click="item.click">
