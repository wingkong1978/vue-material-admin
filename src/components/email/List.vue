<template>
  <v-container id="mailList" fluid fill-height class="pa-0 mail-list">
    <v-layout column class="mail-list--layout">
      <v-toolbar fixed app class="elevation-1 mail-list--toolbar">
        <v-checkbox row hide-details class="check-all" />
        <v-menu offset-y origin="center center" :nudge-bottom="0" transition="scale-transition">
          ...
          <v-list class="pa-0">
            <v-list-tile v-for="(item,index) in mailActions" :key="index" :to="!item.href ? { name: item.name } : null" :href="item.href" ripple="ripple" :disabled="item.disabled" :target="item.target" rel="noopener" @click="item.click">
